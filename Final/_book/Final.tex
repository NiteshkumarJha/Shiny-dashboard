...
\hypersetup{unicode=true,
            pdftitle={A Comparative study on Indian Rainfall data using exploratory analysis and evaluating different time series models},
            pdfauthor={Niteshkumar Jha  Akshay Jadhav},
            pdfborder={0 0 0},
            breaklinks=true}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{list.of.packages <-}\KeywordTok{c}\NormalTok{(}\StringTok{"data.table"}\NormalTok{,}\StringTok{"dplyr"}\NormalTok{,}\StringTok{"plyr"}\NormalTok{,}\StringTok{"sqldf"}\NormalTok{,}
                     \StringTok{"reshape"}\NormalTok{,}\StringTok{"pastecs"}\NormalTok{,}\StringTok{"psych"}\NormalTok{,}\StringTok{"tidyquant"}\NormalTok{,}
                     \StringTok{"nortest"}\NormalTok{,}\StringTok{"tseries"}\NormalTok{,}\StringTok{"ggthemes"}\NormalTok{,}\StringTok{"ggplot2"}\NormalTok{,}
                     \StringTok{"scales"}\NormalTok{,}\StringTok{"forecast"}\NormalTok{,}\StringTok{"Metrics"}\NormalTok{,}\StringTok{"kableExtra"}\NormalTok{,}
                     \StringTok{"pander"}\NormalTok{,}\StringTok{"tidyverse"}\NormalTok{,}\StringTok{"lubridate"}\NormalTok{,}\StringTok{"timeSeries"}\NormalTok{,}
                     \StringTok{"magrittr"}\NormalTok{,}\StringTok{"recipes"}\NormalTok{)}
...
\NormalTok{pander}\OperatorTok{::}\KeywordTok{pander}\NormalTok{(}\StringTok{"All libraries are updated and installed"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
For calculating all above statistics, we are using \textbf{stat.desc} from \texttt{pastecs}.
...
\CommentTok{# using stat.desc}
\KeywordTok{kable}\NormalTok{(}\KeywordTok{stat.desc}\NormalTok{(rainfall[,}\DecValTok{2}\OperatorTok{:}\DecValTok{18}\NormalTok{],}\DataTypeTok{norm =} \OtherTok{TRUE}\NormalTok{), }\StringTok{"latex"}\NormalTok{,}\DataTypeTok{longtable=}\NormalTok{T,}\DataTypeTok{booktabs=}\NormalTok{T,}
      \DataTypeTok{caption=}\StringTok{"Descriptive statistics"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"striped"}\NormalTok{, }\StringTok{"scale_down"}\NormalTok{),}\DataTypeTok{font_size =} \DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Normal distribution line in histogram}
\KeywordTok{hist}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{ANN, }\DataTypeTok{freq=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{col=}\StringTok{"skyblue"}\NormalTok{, }\DataTypeTok{xlab=}\StringTok{"Annual Rainfall"}\NormalTok{, }\DataTypeTok{main=} \StringTok{"Histogram of Annual Rainfall"}\NormalTok{)}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dnorm}\NormalTok{(x, }\DataTypeTok{mean=}\KeywordTok{mean}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{ANN), }\DataTypeTok{sd=}\KeywordTok{sd}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{ANN)), }\DataTypeTok{add=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{col=}\StringTok{"red"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{Final_files/figure-latex/unnamed-chunk-8-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{boxplot}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{ANN,}\DataTypeTok{col =} \StringTok{"pink"}\NormalTok{,}\DataTypeTok{ylab=}\StringTok{"Annual Rainfall"}\NormalTok{,}
        \DataTypeTok{main=} \StringTok{"Box-plot of Annual Rainfall"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{Final_files/figure-latex/unnamed-chunk-9-1} 
...
\begin{Highlighting}[]
\CommentTok{# Normal distribution line in histogram}
\KeywordTok{hist}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{JJAS, }\DataTypeTok{freq=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{col=}\StringTok{"skyblue"}\NormalTok{, }\DataTypeTok{xlab=}\StringTok{"Monsoon Rainfall"}\NormalTok{, }\DataTypeTok{main=} \StringTok{"Histogram of Monsoon Rainfall"}\NormalTok{)}
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dnorm}\NormalTok{(x, }\DataTypeTok{mean=}\KeywordTok{mean}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{JJAS), }\DataTypeTok{sd=}\KeywordTok{sd}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{JJAS)), }\DataTypeTok{add=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{col=}\StringTok{"red"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{Final_files/figure-latex/unnamed-chunk-10-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{boxplot}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{JJAS,}\DataTypeTok{col =} \StringTok{"pink"}\NormalTok{,}\DataTypeTok{ylab=}\StringTok{"Monsoon Rainfall"}\NormalTok{,}
        \DataTypeTok{main=} \StringTok{"Box-plot of Monsoon Rainfall"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{Final_files/figure-latex/unnamed-chunk-11-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rainfall}\OperatorTok{%>%}\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(YEAR,ANN))}\OperatorTok{+}\KeywordTok{geom_line}\NormalTok{()}\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.5}\NormalTok{, }\DataTypeTok{color =} \KeywordTok{palette_light}\NormalTok{()[[}\DecValTok{1}\NormalTok{]], }\DataTypeTok{shape=}\DecValTok{20}\NormalTok{,}\DataTypeTok{size=}\DecValTok{2}\NormalTok{) }\OperatorTok{+}
\StringTok{   }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Annual Rainfall in India"}\NormalTok{, }\DataTypeTok{x =} \StringTok{"YEAR"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Annual Rainfall"}\NormalTok{,}
       \DataTypeTok{subtitle =} \StringTok{"data from 1871-2016"}\NormalTok{) }\OperatorTok{+}
\StringTok{    }\KeywordTok{theme_tq}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-15-1} 

}

\caption{Historical trend of annual rainfall}\label{fig:unnamed-chunk-15}
\end{figure}
...
\tightlist
\item
  The above figure 6.1 shown the seasonality is present in the data.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Calculating Mean for ANN}
\NormalTok{ANN_mean =}\StringTok{ }\KeywordTok{mean}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{ANN)}

\CommentTok{#Calculating percentage departure From Mean for ANN}
\NormalTok{rainfall}\OperatorTok{$}\NormalTok{Perc_departure_ANN =}\StringTok{ }\NormalTok{((rainfall}\OperatorTok{$}\NormalTok{ANN}\OperatorTok{-}\NormalTok{ANN_mean)}\OperatorTok{/}\NormalTok{ANN_mean)}\OperatorTok{*}\DecValTok{100}

\CommentTok{#plotting combo-chart for ANN}
\NormalTok{rainfall1 <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(rainfall, Perc_departure_ANN}\OperatorTok{>=}\StringTok{ }\DecValTok{0}\NormalTok{)}
\NormalTok{rainfall2 <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(rainfall, Perc_departure_ANN }\OperatorTok{<}\StringTok{ }\DecValTok{0}\NormalTok{)}

\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}\StringTok{ }
\StringTok{     }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall1, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{YEAR, }\DataTypeTok{y=}\NormalTok{Perc_departure_ANN),}
              \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{,}\DataTypeTok{colour =} \StringTok{"lightgreen"}\NormalTok{) }\OperatorTok{+}
\StringTok{     }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall2, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{YEAR, }\DataTypeTok{y=}\NormalTok{Perc_departure_ANN),}
              \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{,}\DataTypeTok{colour =} \StringTok{"pink"}\NormalTok{)  }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_ma}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{ YEAR, }\DataTypeTok{y=}\NormalTok{ Perc_departure_ANN),}
          \DataTypeTok{ma_fun =}\NormalTok{ SMA, }\DataTypeTok{n =} \DecValTok{11}\NormalTok{, }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{,}\DataTypeTok{size =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-16-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{series.end <-}\StringTok{ }\KeywordTok{floor}\NormalTok{(}\FloatTok{0.8}\OperatorTok{*}\KeywordTok{nrow}\NormalTok{(rainfall)) }\CommentTok{#select the first 80% of the data}
\NormalTok{train <-}\StringTok{ }\NormalTok{rainfall[}\DecValTok{1}\OperatorTok{:}\NormalTok{series.end,] }\CommentTok{#assign the first 80% of the data to the train set}
\NormalTok{test <-}\StringTok{ }\NormalTok{rainfall[(series.end}\OperatorTok{+}\DecValTok{1}\NormalTok{)}\OperatorTok{:}\KeywordTok{nrow}\NormalTok{(rainfall),] }\CommentTok{#assign the most recent 20% to the test set}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# plot(SimpleExpSmooth)}

\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(SimpleExpSmooth}\OperatorTok{$}\NormalTok{fitted)}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train_ANN[}\DecValTok{2}\OperatorTok{:}\DecValTok{116}\NormalTok{, ]}
\NormalTok{train_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_ANN, train_forecast)}
\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{xhat <-}\StringTok{ }\OtherTok{NULL}
\KeywordTok{setnames}\NormalTok{(train_ANN, }\StringTok{"level"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}

\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{Error =}\StringTok{ }\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{^}\DecValTok{2}
\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{ANN}


\NormalTok{SampleSize =}\StringTok{ }\KeywordTok{nrow}\NormalTok{(train_ANN)}
\NormalTok{MSE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{Error)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{RMSE =}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(MSE)}
\NormalTok{MAE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{AbsError)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{MAPE =}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(train_ANN}\OperatorTok{$}\NormalTok{APE)}\OperatorTok{/}\NormalTok{SampleSize)}\OperatorTok{*}\DecValTok{100}

\NormalTok{statistics =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"SampleSize"}\NormalTok{, }\StringTok{"MSE"}\NormalTok{, }\StringTok{"RMSE"}\NormalTok{, }\StringTok{"MAE"}\NormalTok{, }\StringTok{"MAPE"}\NormalTok{)}
\NormalTok{Value =}\StringTok{ }\KeywordTok{rbind}\NormalTok{(SampleSize, MSE, RMSE, MAE, MAPE)}

\NormalTok{train_statistics <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(statistics, Value))}
\NormalTok{train_statistics}\OperatorTok{$}\NormalTok{training_set <-}\StringTok{ }\NormalTok{train_statistics}\OperatorTok{$}\NormalTok{V2}
\NormalTok{train_statistics}\OperatorTok{$}\NormalTok{V2 <-}\StringTok{ }\OtherTok{NULL}
\KeywordTok{rownames}\NormalTok{(train_statistics) <-}\StringTok{  }\OtherTok{NULL}


\KeywordTok{library}\NormalTok{(forecast)}
\NormalTok{prediction_SES <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(SimpleExpSmooth, }\DataTypeTok{h=}\DecValTok{30}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(prediction_SES)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:unnamed-chunk-20}Forecasted value using simple expontional smoothing for Annual Rainfall}
\end{figure}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(prediction_SES}\OperatorTok{$}\NormalTok{mean)}

\NormalTok{test_ANN =}\StringTok{ }\NormalTok{test[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(test_ANN, Forecast)}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{data.table}\NormalTok{(test_ANN)}
\KeywordTok{setnames}\NormalTok{(test_ANN, }\StringTok{'x'}\NormalTok{, }\StringTok{'Forecast'}\NormalTok{)}

\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Error =}\StringTok{ }\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{^}\DecValTok{2}
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{ANN}

\CommentTok{#kable(test_ANN, "latex",longtable=T,booktabs=T,caption="Simple Exponential Smoothing forecast of test data") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}

\NormalTok{SampleSize =}\StringTok{ }\KeywordTok{nrow}\NormalTok{(test_ANN)}
\NormalTok{MSE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{Error)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{RMSE =}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(MSE)}
\NormalTok{MAE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{AbsError)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{MAPE =}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{APE)}\OperatorTok{/}\NormalTok{SampleSize)}\OperatorTok{*}\DecValTok{100}
...
\begin{Highlighting}[]
\CommentTok{# Plotting the estimates}
\CommentTok{#plot(ExpSmooth)}

\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(ExpSmooth}\OperatorTok{$}\NormalTok{fitted)}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train_ANN[}\DecValTok{3}\OperatorTok{:}\DecValTok{116}\NormalTok{, ]}
\NormalTok{train_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_ANN, train_forecast)}
\NormalTok{train_ANN}\OperatorTok{$}\NormalTok{xhat <-}\StringTok{ }\OtherTok{NULL}
\KeywordTok{setnames}\NormalTok{(train_ANN, }\StringTok{"level"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\KeywordTok{library}\NormalTok{(forecast)}
\NormalTok{prediction_SES <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(ExpSmooth, }\DataTypeTok{h=}\DecValTok{30}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(prediction_SES)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:unnamed-chunk-21}Forecasted value using expontional smoothing for Annual Rainfall}
\end{figure}
...
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{ANN}
\CommentTok{#kable(test_ANN, "latex",longtable=T,booktabs=T,caption="Exponential Smoothing forecast of test data") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-22-1} 
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-23-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# calculate train accuracy}
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{fitted}\NormalTok{(ar3))}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\CommentTok{#train_ANN = train_ANN[3:116, ]}
\NormalTok{train_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_ANN, train_forecast)}
\KeywordTok{setnames}\NormalTok{(train_ANN, }\StringTok{"x"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\CommentTok{# forecasting for test data}
\NormalTok{ar3_forecast <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(ar3,}\DataTypeTok{h =} \DecValTok{30}\NormalTok{)}

\CommentTok{# plotting the forecasted series}
\KeywordTok{plot}\NormalTok{(ar3_forecast)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:unnamed-chunk-24}Forecasted value using AR model for Annual Rainfall}
\end{figure}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# calculate test accuracy}
\NormalTok{Forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(ar3_forecast}\OperatorTok{$}\NormalTok{mean)}
\NormalTok{test_ANN =}\StringTok{ }\NormalTok{test[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(test_ANN, Forecast)}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{data.table}\NormalTok{(test_ANN)}
\KeywordTok{setnames}\NormalTok{(test_ANN, }\StringTok{'x'}\NormalTok{, }\StringTok{'Forecast'}\NormalTok{)}
...
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{ANN}
\CommentTok{#kable(test_ANN, "latex",longtable=T,booktabs=T,caption="ARIMA Model of forecasting") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
...
\KeywordTok{kable}\NormalTok{(Finalresult_ar3, }\StringTok{"latex"}\NormalTok{,}\DataTypeTok{longtable=}\NormalTok{T,}\DataTypeTok{booktabs=}\NormalTok{T,}\DataTypeTok{caption=}\StringTok{"Accuracy Metrics of Annual Rainfall using AR Model"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"striped"}\NormalTok{, }\StringTok{"scale_down"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{longtable}{lll}
\caption{\label{tab:unnamed-chunk-24}Accuracy Metrics of Annual Rainfall using AR Model}\\
\toprule
statistics & training\_set & testing\_set\\
\midrule
\rowcolor{gray!6}  MAE & 79.5272908037679 & 74.678392225741\\
MAPE & 7.45444051321105 & 7.19660077964034\\
\rowcolor{gray!6}  MSE & 10494.6574654656 & 8474.68080698905\\
...
\begin{Highlighting}[]
\CommentTok{# calculate train accuracy}
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(NN}\OperatorTok{$}\NormalTok{fitted)}
\NormalTok{train_ANN =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\CommentTok{#train_ANN = train_ANN[2:116, ]}
\NormalTok{train_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_ANN, train_forecast)}
\KeywordTok{setnames}\NormalTok{(train_ANN, }\StringTok{"x"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\CommentTok{# forecasting for test data}
\NormalTok{NN_forecast <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(NN,}\DataTypeTok{h =} \DecValTok{30}\NormalTok{)}

\CommentTok{# plotting the forecasted series}
\KeywordTok{plot}\NormalTok{(NN_forecast)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:unnamed-chunk-25}Forecasted value using Neural network for Annual Rainfall}
\end{figure}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# calculate test accuracy}
\NormalTok{Forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(NN_forecast}\OperatorTok{$}\NormalTok{mean)}
\NormalTok{test_ANN =}\StringTok{ }\NormalTok{test[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"ANN"}\NormalTok{)]}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(test_ANN, Forecast)}
\NormalTok{test_ANN =}\StringTok{ }\KeywordTok{data.table}\NormalTok{(test_ANN)}
\KeywordTok{setnames}\NormalTok{(test_ANN, }\StringTok{'x'}\NormalTok{, }\StringTok{'Forecast'}\NormalTok{)}
...
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_ANN}\OperatorTok{$}\NormalTok{ANN }\OperatorTok{-}\StringTok{ }\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_ANN}\OperatorTok{$}\NormalTok{ANN}
\CommentTok{#kable(test_ANN, "latex",longtable=T,booktabs=T,caption="Neural Network Modals of forecasting") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ rainfall}\OperatorTok{%>%}\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(YEAR,JJAS))}\OperatorTok{+}\KeywordTok{geom_line}\NormalTok{()}\OperatorTok{+}
\StringTok{   }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.5}\NormalTok{, }\DataTypeTok{color =} \KeywordTok{palette_light}\NormalTok{()[[}\DecValTok{1}\NormalTok{]], }\DataTypeTok{shape=}\DecValTok{20}\NormalTok{,}\DataTypeTok{size=}\DecValTok{2}\NormalTok{) }\OperatorTok{+}
\StringTok{    }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"JJAS Rainfall in India"}\NormalTok{, }\DataTypeTok{x =} \StringTok{"YEAR"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"JJAS Rainfall"}\NormalTok{,}
        \DataTypeTok{subtitle =} \StringTok{"data from 1871-2016"}\NormalTok{) }\OperatorTok{+}
\StringTok{     }\KeywordTok{theme_tq}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-26-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Calculating Mean for JJAS}
\NormalTok{JJAS_mean =}\StringTok{ }\KeywordTok{mean}\NormalTok{(rainfall}\OperatorTok{$}\NormalTok{JJAS)}

\CommentTok{#Calculating percentage departure From Mean for JJAS}
\NormalTok{rainfall}\OperatorTok{$}\NormalTok{Perc_departure_JJAS =}\StringTok{ }\NormalTok{((rainfall}\OperatorTok{$}\NormalTok{JJAS}\OperatorTok{-}\NormalTok{JJAS_mean)}\OperatorTok{/}\NormalTok{JJAS_mean)}\OperatorTok{*}\DecValTok{100}

\CommentTok{#plotting combo-chart for ANN}
\NormalTok{rainfall1 <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(rainfall, Perc_departure_JJAS}\OperatorTok{>=}\StringTok{ }\DecValTok{0}\NormalTok{)}
\NormalTok{rainfall2 <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(rainfall, Perc_departure_JJAS }\OperatorTok{<}\StringTok{ }\DecValTok{0}\NormalTok{)}

\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}\StringTok{ }
\StringTok{     }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall1, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{YEAR, }\DataTypeTok{y=}\NormalTok{Perc_departure_JJAS),}
              \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{,}\DataTypeTok{colour =} \StringTok{"lightgreen"}\NormalTok{) }\OperatorTok{+}
\StringTok{     }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall2, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{YEAR, }\DataTypeTok{y=}\NormalTok{Perc_departure_JJAS),}
              \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{,}\DataTypeTok{colour =} \StringTok{"pink"}\NormalTok{)  }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_ma}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ rainfall,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{ YEAR, }\DataTypeTok{y=}\NormalTok{ Perc_departure_JJAS),}
          \DataTypeTok{ma_fun =}\NormalTok{ SMA, }\DataTypeTok{n =} \DecValTok{11}\NormalTok{, }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{,}\DataTypeTok{size =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-27-1} 

}

\caption{Combo chart of monsoon rainfall}\label{fig:unnamed-chunk-27}
\end{figure}
...
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(SimpleExpSmooth}\OperatorTok{$}\NormalTok{fitted)}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"JJAS"}\NormalTok{)]}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train_JJAS[}\DecValTok{2}\OperatorTok{:}\DecValTok{116}\NormalTok{, ]}
\NormalTok{train_JJAS =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_JJAS, train_forecast)}
\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{xhat <-}\StringTok{ }\OtherTok{NULL}
\KeywordTok{setnames}\NormalTok{(train_JJAS, }\StringTok{"level"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}

\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{Error =}\StringTok{ }\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{^}\DecValTok{2}
\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{JJAS}

\NormalTok{SampleSize =}\StringTok{ }\KeywordTok{nrow}\NormalTok{(train_JJAS)}
\NormalTok{MSE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{Error)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{RMSE =}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(MSE)}
\NormalTok{MAE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{AbsError)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{MAPE =}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(train_JJAS}\OperatorTok{$}\NormalTok{APE)}\OperatorTok{/}\NormalTok{SampleSize)}\OperatorTok{*}\DecValTok{100}
...
\caption{\label{fig:unnamed-chunk-31}Forecasted value using simple expontional smoothing for Monsoon Rainfall}
\end{figure}
...
\NormalTok{test_JJAS =}\StringTok{ }\NormalTok{test[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"JJAS"}\NormalTok{)]}
\NormalTok{test_JJAS =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(test_JJAS, Forecast)}
\NormalTok{test_JJAS =}\StringTok{ }\KeywordTok{data.table}\NormalTok{(test_JJAS)}
\KeywordTok{setnames}\NormalTok{(test_JJAS, }\StringTok{'x'}\NormalTok{, }\StringTok{'Forecast'}\NormalTok{)}

\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Error =}\StringTok{ }\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{^}\DecValTok{2}
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{JJAS}
\CommentTok{#kable(test_JJAS, "latex",longtable=T,booktabs=T,caption="Simple Exponential Smoothing forecast of test data") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}

\NormalTok{SampleSize =}\StringTok{ }\KeywordTok{nrow}\NormalTok{(test_JJAS)}
\NormalTok{MSE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{Error)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{RMSE =}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(MSE)}
\NormalTok{MAE =}\StringTok{ }\KeywordTok{sum}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{AbsError)}\OperatorTok{/}\NormalTok{SampleSize}
\NormalTok{MAPE =}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{APE)}\OperatorTok{/}\NormalTok{SampleSize)}\OperatorTok{*}\DecValTok{100}
...
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(ExpSmooth}\OperatorTok{$}\NormalTok{fitted)}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"JJAS"}\NormalTok{)]}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train_JJAS[}\DecValTok{1}\OperatorTok{:}\DecValTok{116}\NormalTok{, ]}
\NormalTok{train_JJAS =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_JJAS, train_forecast)}
\NormalTok{train_JJAS}\OperatorTok{$}\NormalTok{xhat <-}\StringTok{ }\OtherTok{NULL}
\KeywordTok{setnames}\NormalTok{(train_JJAS, }\StringTok{"x"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\begin{figure}
\centering
\includegraphics{Final_files/figure-latex/unnamed-chunk-32-1.pdf}
\caption{\label{fig:unnamed-chunk-32}Forecasted value using expontional smoothing for Monsoon Rainfall}
\end{figure}
...
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{JJAS}
\CommentTok{#kable(test_JJAS, "latex",longtable=T,booktabs=T,caption="Exponential Smoothing forecast of test data") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-33-1} 
...
{\centering \includegraphics[width=0.75\linewidth]{Final_files/figure-latex/unnamed-chunk-34-1} 
...
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{fitted}\NormalTok{(ar3))}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"JJAS"}\NormalTok{)]}
\CommentTok{#train_JJAS = train_JJAS[3:116, ]}
\NormalTok{train_JJAS =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_JJAS, train_forecast)}
\KeywordTok{setnames}\NormalTok{(train_JJAS, }\StringTok{"x"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\caption{\label{fig:unnamed-chunk-35}Forecasted value using AR model for Monsoon Rainfall}
\end{figure}
...
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{JJAS}
\CommentTok{#kable(test_JJAS, "latex",longtable=T,booktabs=T,caption="ARIMA Model of forecasting") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
...
\begin{Highlighting}[]
\CommentTok{# calculate train accuracy}
\NormalTok{train_forecast =}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{fitted}\NormalTok{(NN))}
\NormalTok{train_JJAS =}\StringTok{ }\NormalTok{train[,}\KeywordTok{c}\NormalTok{(}\StringTok{"YEAR"}\NormalTok{, }\StringTok{"JJAS"}\NormalTok{)]}
\NormalTok{train_JJAS =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(train_JJAS, train_forecast)}
\KeywordTok{setnames}\NormalTok{(train_JJAS, }\StringTok{"x"}\NormalTok{, }\StringTok{"Forecast"}\NormalTok{)}
...
\caption{\label{fig:unnamed-chunk-36}Forecasted value using Neural network for Monsoon Rainfall}
\end{figure}
...
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{AbsError =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}
\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{APE =}\StringTok{ }\KeywordTok{abs}\NormalTok{(test_JJAS}\OperatorTok{$}\NormalTok{JJAS }\OperatorTok{-}\StringTok{ }\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{Forecast)}\OperatorTok{/}\NormalTok{test_JJAS}\OperatorTok{$}\NormalTok{JJAS}
\CommentTok{#kable(test_JJAS, "latex",longtable=T,booktabs=T,caption="Neural Network Modals of forecasting") %>%}
\CommentTok{#  kable_styling(latex_options = c("striped", "scale_down"))}
